...
% chance of disappearing
  ...
  for i=1:length(oldParticles)
      try
    weightSum = weightSum + oldParticles(i).landmarks(1).w;
      catch
          oldParticles(i).landmarks(1).w=0;
        weightSum = weightSum + oldParticles(i).landmarks(1).w;
  ...
  for i=1:length(oldParticles)
      ...
      oldParticles(i).landmarks(1).w = oldParticles(i).landmarks(1).w/weightSum;
  ...
  c = oldParticles(1).landmarks(1).w;
  ...
  for m=1:M
    ...
    while U > c
      ...
      c = c+oldParticles(i).landmarks(1).w;
  ...
  for i=1:length(newParticles)
      ...
      newParticles(i).landmarks(1).w = 1.0/length(newParticles);

...
avg_robot_position = [0;      % x %Use UTM coordinates from GPS
    0;     % y
    0];  % rotation
...
for timestep = 2:timesteps
    ...
    if isempty(z_real)==0
        doResample=true; %if there is not new measurement, don't resample particles
    else
        continue;
    end
    ...
    for pIdx = 1:num_particles
        ...
        for j=1:particles(pIdx).num_landmarks(timestep) %length(data_associate_vect)
            ...
            elseif (data_associate_vect(j)>0 && j<=particles(pIdx).num_landmarks(timestep-1))
                %we've seen this landmark before
